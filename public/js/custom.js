$(document).ready(function(){
  $("#id_subtypes>select.select2").select2({
    tags: true,
    tokenSeparators: [',', ' ']
  })
  $(document).on("change", ".select-country", function(){
         ...
         $.ajax({
              method: 'POST',
              url: '/getCounties',
              data: {_token: '{{csrf_token()}}', country_code: $(this).val()},
              context: this,
              async: true,
              cache: false,
              dataType: 'json'
      ...
      $(document).on("change", ".select-state", function(){
         ...
         $.ajax({
              method: 'POST',
              url: '/getCities',
              data: {_token: '{{csrf_token()}}', state_code: $(this).val(), country_code: $(vthis).parent().parent().find(".select-country").val()},
              context: this,
              async: true,
              cache: false,
              dataType: 'json'
