...
//         // insert/update data into individuals/legal_entities (fizica/juridica)
        ...
        $editInsertAddress->phone = $editInsertAddress->delivery_phone != null ? $editInsertAddress->delivery_phone : $editInsertAddress->userData(
        )->phone;
        $editInsertAddress->name = $editInsertAddress->delivery_contact != null ? $editInsertAddress->delivery_contact : $editInsertAddress->userData(
        )->name;
        return [
            'success' => true,
            'userAddresses' => [0 => $editInsertAddress],
            'transparent_band' => 0,
            'client_id' => $client->id,
    ...
    public function retrievePreselectedColors(Request $request)
    {
        return [
            'success' => true,
            'colors' => (new self())->updateOfferAttributeForPreselectedColor(
                $request->input('selectedColor'),
                $request->input('offerId'),
                $request->input('offerType')
            )
        ];
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->input('delivery_address_user') != null) {
            ...
            $data->reducere = $request->input('reducere') != null ? number_format(
                floatval(abs($request->input('reducere'))),
                2,
                '.',
                ''
            ) : 0;
            $data->total_final = $request->input('totalCalculatedPrice') != null ? number_format(
...
//             $redirect = redirect()->route("voyager.{$dataType->slug}.index");
    ...
    public function edit(Request $request, $id)
    {
        ...
        $cursValutar = $offer->curs_eur != null ? $offer->curs_eur : ($offerType->exchange != null ? $offerType->exchange : \App\Http\Controllers\Admin\CursBNR::getExchangeRate(
            "EUR"
        ));
        ...
        if ($selectedAddress != null) {
            ...
            $selectedAddress->phone = $selectedAddress->delivery_phone != null ? $selectedAddress->delivery_phone : $selectedAddress->userData(
            )->phone;
            $selectedAddress->name = $selectedAddress->delivery_contact != null ? $selectedAddress->delivery_contact : $selectedAddress->userData(
            )->name;
        }
        ...
        if ($offerType->parents && count($offerType->parents) > 0) {
            ...
            if (count($prods) > 0) {
                ...
                $colors = DB::select(
                    DB::raw(
                        'SELECT DISTINCT product_attributes.id, CONCAT(attributes.title, "_", attributes.id) as attr_title, CONCAT(colors.value, "_", colors.id) as color_value, colors.ral as color_ral
                ...
                $dimensions = DB::select(
                    DB::raw(
                        'SELECT DISTINCT CONCAT(attributes.title, "_", attributes.id) as attr_title, CONCAT(dimensions.value, ";_", dimensions.id) as dimension_value
                          FROM product_attributes
                          JOIN attributes ON product_attributes.attribute_id = attributes.id
        ...
        $adminUsers = User::get();
        $offerEvents = OfferEvent::where('offer_id', $offer->id)->where('is_mention', 0)->orderBy(
            'created_at',
            'DESC'
        )->get();
        $offerMessages = OfferEvent::where('offer_id', $offer->id)->where('is_mention', 1)->orderBy(
            'created_at',
            'DESC'
        )->get();
    ...
    public function retrievePricesForSelectedAttributes($order_id, $attributes, $modifyOfferProductsPrices)
    {
        ...
        if ($offerProducts && count($offerProducts) > 0 && $modifyOfferProductsPrices) {
            ...
            OfferProduct::whereIn('id', $offProdIds)->delete(
            ); // sterg toate valorile pentru ca am produse noi, definite prin atributele selectate
            OfferPrice::whereIn('offer_products_id', $offProdIds)->delete(
            ); // sterg toate valorile pentru ca am produse noi, definite prin atributele selectate
            foreach ($offerProducts as $offProd) {
                $arrElem = [
                    'offer_id' => $offer->id,
        ...
        if (count($attrQueryArray) > 0) {
            ...
            foreach ($offerType->parents as $parent) {
                ...
                $nrOfAttrs = count($attributes) >= count($parent->category->attributes) ? count(
                    $parent->category->attributes
                ) : count($attributes);
                // iau atributele
                $productAttrs = ProductAttribute::select(
                    "*",
                    DB::raw('IF(COUNT(product_id) = ' . $nrOfAttrs . ', true, false) as founded')
        ...
        if ($modifyOfferProductsPrices) {
            ...
            foreach ($products as $product) {
                $rulesPrices = (new self())->getRulesPricesByProductCategory(
                    $product->categoryId(),
                    $product->price,
                    $cursValutar
                );
    ...
    public function ajaxSaveUpdateOffer(Request $request)
    {
        ...
        if ($offer_id != null) {
            ...
            $offerDb = Offer::find(
                $offer_id
            ); // mai fac un query pentru ca daca echivalez offerDb cu offer, orice modificare aduc lui offer, se propaga si la offerDb
        } else {
            $offer = new Offer;
        }
        ...
        */
        ...
        if ($request->input('getPrices')) {
            $trievedPrices = (new self())->retrievePricesForSelectedAttributes(
                $offer->id,
                $selectedAttributes,
                $modifyOfferProductsPrices
            );
            return [
                'success' => true,
                'offer_id' => $offer->id,
                'html_log' => (new self())->getHtmlLog($offer),
                'html_prices' => $trievedPrices
            ];
        } else {
        }
...
//                          $dimension += $offProd->getParent->dimension != null && $offProd->getParent->dimension != 0 ? $offProd->getParent->dimension*$offProd->qty : $offProd->qty;
    ...
    public function retrieveOffersPerYearMonth(Request $request)
    {
        ...
        $calendarOrders = Offer::with('status_name')->select('offer_date', 'serie', 'status')->whereRaw(
            'YEAR(offer_date) = ' . $year . ' AND MONTH(offer_date) = ' . $month
        )->get();
    ...
    public function changeStatus(Request $request)
    {
        ...
        try {
            ...
            return [
                'success' => true,
                'msg' => 'Statusul a fost modificat cu succes!',
                'html_log' => (new self())->getHtmlLog($offer)
            ];
        } catch (\Exception $e) {
    ...
    public function launchOrder(Request $request)
    {
        ...
        try {
            ...
            if ($checkSync['success'] == true) {
                ...
                return [
                    'success' => true,
                    'msg' => 'Comanda a fost lansata cu succes!',
                    'status' => $status->title,
                    'html_log' => (new self())->getHtmlLog($offer),
                    'numar_comanda' => $nextOrderNumber
                ];
            } else {
            ...
            if ($taggedUsers != null) {
                ...
                foreach ($adminUsers as $user) {
                    Mail::to($user->email)->send(
                        new Mentions($offer->id, $offer->id, $message, Auth::user()->name, $user->name, false)
                    ); // trimite catre userii tag-uiti
                }
                $adminEmails = explode(" ", setting('admin.cc_emails'));
                foreach ($adminEmails as $email) {
                    Mail::to($email)->send(
                        new Mentions($offer->id, $offer->id, $message, Auth::user()->name, 'Admin', true)
                    ); // trimite catre email-urile din admin
                }
            }

            return [
                'success' => true,
                'msg' => 'Mesaj salvat cu succes!',
                'html_log' => (new self())->getHtmlLog($offer),
                'html_messages' => (new self())->getHtmlLogMentions($offer->id)
            ];
        } catch (\Exception $e) {
...
//   	}
    ...
    public static function replaceDataInTemplate($offer)
    {
        ...
        if ($courierObj == null) {
            return [
                'success' => false,
                'msg' => 'Nu se poate trimite SMS pentru ca nu s-a generat un AWB pentru aceasta comanda!'
            ];
        }
    ...
    public static function syncOrderToWinMentor($order_id)
    {
        ...
        $url = "http://" . config('winmentor.host') . ":" . config(
                'winmentor.port'
            ) . "/datasnap/rest/TServerMethods/ComandaClient//";
        ...
        if ($winMentorServer) {
            ...
            return [
                'success' => false,
                'msg' => '[WinMentor] Nu s-a putut conecta la serverul WinMentor!',
                'warning' => false
            ];
        }
        curl_close($ch);
        if (array_key_exists('Error', $result) && $result['Error'] == "ok" && $winMentorServer) {
            ...
            $eroare = array_key_exists(
                'error',
                $result
            ) ? '[WinMentor] ' . $result['error'] : '[WinMentor] ' . $result['Error'];
    ...
    public function getColorsByOfferType(Request $request, $offerTypeId = null)
    {
        ...
        $selectedColors = OffertypePreselectedColor::with('color')->where('offer_type_id', $offerTypeId)->groupBy(
            'color_id'
        )->get();
        $html_colors = '';
