        ...
        if ($request->input('country_code') != null) {
            foreach ($counties as $state) {
                if ($state->country_code == $request->input('country_code')) {
                    ...
                    if ($request->input('selected_state') != null && $request->input(
                            'selected_state'
                        ) == $state->state_code . '_' . $state->state_name) {
...
//       cache()->flush();exit;
        ...
        if ($cities != null) {
            foreach ($cities as $city) {
                ...
                if ($request->input('selected_city') != null && $request->input(
                        'selected_city'
                    ) == $city->id . '_' . $city->city_name) {
    ...
    public static function getCities($stateCode, $countryCode)
    {
        ...
        $cities = DB::table('cities')->where(['state_code' => $stateCode, 'country_code' => $countryCode])->orderBy(
            'city_name'
        )->get();
    ...
    public function saveNewAddress(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return [
                ...
                'msg' => 'Adresa a fost modificata cu succes!'
            ];
        }
    }
}
